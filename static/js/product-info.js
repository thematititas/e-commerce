...
document.addEventListener("DOMContentLoaded", function (e) {
    ...
    document.getElementById("comprar").addEventListener("click", function () {
        ...
        postJSONData(NEW_CART,precio).then(function (result){
            if(result.status === "ok"){
        ...
        if (localStorage.getItem('nameUsuario')) { // if estas logeado
            if (document.getElementById("texto").value) { // if comentaste
                if (contador) {// if calificaste
                    comentario = newObject(document.getElementById("texto").value)
                    showComentario(comentario);

                    postJSONData(NEW_COMMENT,comentario).then(function (result){
                        if(result.status === "ok"){
                            alert(result.data.mensaje);
                        }
                    });
